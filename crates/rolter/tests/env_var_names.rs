...
/// Find every runtime environment variable name read across the crates' sources.
fn runtime_env_vars() -> BTreeSet<String> {
    let prefixed = prefixed_names();
    prefixed
        .into_iter()
        .filter(|v| {
            !v.starts_with("ROLTER_TEST_")
                && v != "ROLTER_MASTER_KEY"
                && v != "ROLTER_PLUGIN_TOKEN"
                && v != "ROLTER_GUARDRAIL_TOKEN"
                && v != "ROLTER_CORS_ALLOW_ORIGINS"
        })
        .collect()
}
...
fn all_binary_cli_env_vars_are_documented_in_reference() {
    ...
    assert!(
        missing.is_empty(),
        "these environment variables are read by the binaries/workspace but missing from docs/user-docs/configuration/environment-variables.mdx:\n  {}",
        missing.join("\n  ")
    );
}
