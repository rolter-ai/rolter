  ...
  return (
    <aside aria-label={t("pages.promptRepo.versionHistory")} className={cn("overflow-hidden rounded-xl border border-[color:var(--border-subtle)] bg-[color:var(--surface-raised)]", className)}>
      ...
      <div className="max-h-[30rem] space-y-1 overflow-y-auto p-2 2xl:max-h-[calc(100vh-14rem)]">
        {loading ? <LoadingRegion><Skeleton width="100%" height={180} radius={8} data-testid="prompt-versions-loading" /></LoadingRegion> : versions.length === 0 ? <p className="px-2 py-5 text-center text-xs text-muted-foreground">{t("pages.promptRepo.noSavedVersions")}</p> : versions.map((version) => {
