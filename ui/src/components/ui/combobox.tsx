...
export const Combobox = React.forwardRef<HTMLInputElement, ComboboxProps>(function Combobox(
  ...
  const row = ({ option, index }: { option: ComboboxOption; index: number }) => (
    <div
      ...
      style={
        virtual ? { height: ROW[size][option.description ? "described" : "plain"] } : undefined
      }
  ...
  const group = (label: string | undefined, children: React.ReactNode[], header = true) => {
    ...
    return (
      <div
        key={`${label}-${(children[0] as { key?: string })?.key ?? ""}`}
        role="group"
        aria-label={label}
      >
        {header && (
    ...
    >
      ...
      >
        ...
        >
          ...
          {open &&
            !virtual &&
            sections.map((section) => group(section.label, section.items.map(row)))}
