...
Only the exceptions travel. A subsystem absent from the array is stable, which
is what the dashboard renders nothing for — absence is the signal, and
`"stability": "stable"` is never sent. An empty array is the healthy answer for
a release with nothing experimental left in it.

It is its own endpoint rather than a field on an existing payload because the
marker is read by a different question than any of them answers: `feature-flags`
is superadmin-only and about enablement, and `version` is about the update
check. The capability is `stability:read`, held by **every** authenticated
caller including a viewer with no membership anywhere — the nav rail is rendered
for everyone who can sign in, and a marker the least-privileged user cannot
fetch is missing exactly where the reassurance is worth most. It names no
tenant's data, only what this build ships. The operation is `getStability` in
the control-plane OpenAPI document, with a `SubsystemStability` schema, so an
SDK author sees it too.
