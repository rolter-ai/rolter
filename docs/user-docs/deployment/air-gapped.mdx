...
- **Postgres / Redis / ClickHouse** — only when their URLs are set (`ROLTER_DATABASE_URL`, `ROLTER_REDIS_URL`, `CLICKHOUSE_URL` or `[logging].clickhouse_url`).
...
Its documentation links are off unless you configure a documentation host, so an
enclave with no mirror of this site simply sees no links — see
[Documentation links](/deployment/documentation-links).
