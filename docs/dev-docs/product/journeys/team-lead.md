...
| ---- | ---------------------------------- | --------------------------------------------- | --------------------------------------------------------------------------------------------------- | ------------------- |
| T1.1 | sign in (SSO or password)          | login screen                                  | the dashboard, scoped to team `default`                                                             | verified (password) |
...
| T1.3 | see what the team may call         | **Models → Model Catalog**, **Routing Rules** | the routes of the team's projects                                                                   | works               |

## T2 — bring the team in

| #    | step                                               | where                                                                     | expect                                                                  | status          |
| ---- | -------------------------------------------------- | ------------------------------------------------------------------------- | ----------------------------------------------------------------------- | --------------- |
| T2.1 | create a project for a new workstream              | scope switcher **+** under the team · `POST /api/v1/teams/{id}/projects`  | the project, owned by the team                                          | works           |
| T2.2 | invite engineers as members of that project        | **Governance → Users → Invite user**, scope = the project, role = member  | a link per person; authorized at the project, so no org admin is needed | works           |
...
| #    | step                                              | where                                   | expect                                                                                 | status            |
| ---- | ------------------------------------------------- | --------------------------------------- | -------------------------------------------------------------------------------------- | ----------------- |
| T3.1 | route a public model name to an existing provider | **Routing Rules** in the team's project | allowed: routes are project-scoped and the lead is admin above the project             | works             |
| T3.2 | add a new provider                                | **Model Providers → + Add provider**    | refused, naming the Admin role at the org — a request to the platform team             | works (by design) |
| T3.3 | mint a shared key for the team's service          | **Governance → Virtual Keys**           | as [platform-admin A4.2](platform-admin.md#a4--give-access-to-people-and-applications) | works             |

## T4 — keep it inside a budget

| #    | step                                            | where                                                                        | expect                                                                                                      | status      |
| ---- | ----------------------------------------------- | ---------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- | ----------- |
| T4.1 | cap the team's monthly spend                    | **Budgets & Limits → Add budget**, scope = the team · `POST /api/v1/budgets` | the team's requests get HTTP 402 once the month's spend reaches the cap                                     | works       |
| T4.2 | cap each project's throughput                   | **Add rate limit**, scope = the project                                      | HTTP 429 with `Retry-After` past the cap                                                                    | works       |
...
| #    | step                                             | where                                                    | expect                                                   | status      |
| ---- | ------------------------------------------------ | -------------------------------------------------------- | -------------------------------------------------------- | ----------- |
| T6.1 | remove their membership                          | **Governance → Users**, the person's role at the project | they lose the project in the dashboard at once           | works       |
| T6.2 | their personal keys stop working                 | the gateway                                              | a key they minted for themselves is refused              | bug — #1841 |
| T6.3 | shared keys they minted as an admin keep working | the gateway                                              | the team's services do not go down because a person left | works       |
