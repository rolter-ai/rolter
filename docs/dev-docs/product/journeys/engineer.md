...
| E1.2 | find out what models exist | **Models → Model Catalog**     | the project's routes: `gpt-4o`, `llama-3.1-8b`, … with their capabilities                | works    |

## E2 — get a key

| #    | step                                  | where                                                                                                | expect                                                     | status |
| ---- | ------------------------------------- | ---------------------------------------------------------------------------------------------------- | ---------------------------------------------------------- | ------ |
| E2.1 | mint a personal key                   | **Settings → My Virtual Keys → Generate virtual key** · `POST /api/v1/me/projects/{id}/virtual-keys` | the key once, a name, an expiry; a viewer would be refused | works  |
| E2.2 | narrow it to the models the job needs | the mint form's model list                                                                           | `/v1/models` with that key lists only those                | works  |
| E2.3 | rotate or delete it                   | the key card · `POST /api/v1/me/virtual-keys/{id}/rotate`                                            | a new secret; the old one refused                          | works  |

## E3 — make the first call

| #    | step                                             | where                                                           | expect                                          | status   |
| ---- | ------------------------------------------------ | --------------------------------------------------------------- | ----------------------------------------------- | -------- |
| E3.1 | from the Playground, with no key handling at all | **Playground** (mints a 30-minute session key for the project)  | an answer, streamed                             | works    |
...
| E5.2 | find the request                    | **LLM Logs**, filtered by key and status = error; or by the `x-request-id` it returned | the row: status, target, provider, error text, latency, retries, and the captured bodies                | verified (member reads bodies)               |
...
| #    | step                             | where                                               | expect                              | status      |
| ---- | -------------------------------- | --------------------------------------------------- | ----------------------------------- | ----------- |
| E9.1 | usage per key over the last week | **Settings → My Virtual Keys** (`/api/v1/me/usage`) | requests and cost per key           | works       |
...
| E10.4 | protect the account with a second factor    | **Settings → My Virtual Keys → Two-factor authentication** | TOTP and recovery codes                        | works           |
