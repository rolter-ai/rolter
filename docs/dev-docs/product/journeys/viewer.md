...
| V2.2 | enrol a second factor        | **Settings → My Virtual Keys → Two-factor authentication** | TOTP and recovery codes, same as any role               | works       |
...
| #    | step                                      | where                                | expect                                                                                                                       | status |
| ---- | ----------------------------------------- | ------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------- | ------ |
| V4.1 | which models, routes and strategies exist | **Model Catalog**, **Routing Rules** | readable; every edit, delete and toggle disabled, naming the role it needs                                                   | works  |
| V4.2 | which keys exist and what they may reach  | **Governance → Virtual Keys**        | names, prefixes, allow-lists; never a secret                                                                                 | works  |
| V4.3 | which budgets and limits apply            | **Budgets & Limits**                 | readable, not editable                                                                                                       | works  |
| V4.4 | who may do what                           | **Governance → Roles & Permissions** | the published matrix, including this session's `analytics`, `request_payload`, `provider_health` and `project_settings` rows | works  |
