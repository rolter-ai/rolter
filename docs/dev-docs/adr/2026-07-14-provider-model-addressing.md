...
way to say _which_ one it wants. This is common with self-hosted OpenAI-compatible
...
- **LiteLLM** — a two-tier scheme: clients send a public `model_name` alias; each
  ...
  semantically muddy and is _not_ a stable identity.
- **Envoy AI Gateway** — extracts the `model` field from the request body into an
  `x-ai-eg-model` header _before_ routing, then applies ordinary header-match rules in
...
4. **`/v1/models`**: list both — existing route ids _and_ `provider-slug/model` ids
...
unique index" leaves the _behaviour_ around collisions unspecified. This section pins it
...
- _Follow-up_: emit a migration report (one line per adjusted provider: `org_id,
...
- An explicit `slug` is validated against `^[a-z0-9][a-z0-9-]{0,62}$`; an omitted one is
  ...
  explicitly.
- _Follow-up_: map the unique violation to **409** with the next free suggestion
...
- **Collision policy** as in _Slug collision handling_ above: slugs are org-scoped
...
`vllm-10` — where pinning any single one is the _opposite_ of what the operator wants.
...
- **Credentials**: unchanged. Clients authenticate with a rolter virtual key; member
  credentials stay per provider. A group gives _one address, one client key, N upstreams
  with N distinct upstream creds_ — which is the unification the fleet operator actually
  needs.
...
1. **store**: add immutable `slug` to providers — migration, `unique(org_id, slug)`,
   ...
   soft-delete-preferred reclaim, CRUD wiring. (relates to ROL-81, see _Slug collision
   handling_)
...
5. _(optional)_ **proxy**: `x-rolter-provider` header selector (Option D).
6. **store/gateway/ui**: provider groups — `provider_groups` entity (slug in the unified
   ...
   (see _Addendum: provider groups_)
