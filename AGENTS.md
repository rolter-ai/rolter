...
- `ROLTER_TEST_DATABASE_URL` names the **server**; the database is derived per worktree (`rolter_test_wt_<worktree>_<digest>`) by `rolter_store::postgres::test_database` and created on first use, so parallel worktrees never share one (#1430). Resolve it through `test_database::url()` rather than reading the variable — `crates/rolter-store/tests/db_test_isolation.rs` enforces that, plus `TestSchema` for schemas and no process-wide `set_var` in the postgres suites.
- `rolter-control` CRUD tests only build under `--features postgres`. Check both feature sets before pushing.
- `cargo hack check --each-feature --workspace` runs in CI under `RUSTFLAGS=-D warnings`: every feature must compile alone _and warning-free_, so never let a feature-gated item leak into a default-feature path, and gate a helper whose only caller is feature-gated with the same `#[cfg]` rather than leaving it dead under the other combinations (#1399). Reproduce with `RUSTFLAGS="-D warnings" cargo hack check --each-feature --workspace --all-targets`; the default clippy run only covers default features and will not catch these.
...
| Marked a subsystem experimental, or graduated one                | add, edit or remove the `SubsystemStability` row in `crates/rolter-core/src/stability.rs` (keep it sorted by `id`; an `id` is published and never renamed); update the table in `docs/dev-docs/development/stability-markers.md` — the `the_docs_page_lists_exactly_these_subsystems` test compares them row for row; put the note at the top of the subsystem's `docs/user-docs/` page; `nav_keys` are leaf keys from `NAV` in `ui/src/lib/nav.tsx` and are checked against it; add or drop the dashboard's translated note as `stability.notes.<id>` in **every** catalog in `ui/src/lib/i18n/locales/` (`ui/scripts/stability-notes.test.ts` checks the `en` keys against the ids). Graduating belongs in the PR that closes the gap the note names, never in a sweep                                                                                                                                                                                                                                                                                                                                  |
| Added a markdown, MDX, JSON or YAML file outside `ui/`           | run `just fmt-docs` before committing — `docs formatting (prettier)` in `quality.yml` and the `prettier (markdown / json / yaml outside ui/)` prek hook both check it; if the file genuinely cannot be formatted, exclude it in `.prettierignore` with the reason, never by loosening the config                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
- Never let a coding-agent session url (`claude.ai/code/session…`, or an
  ...
  tooling appends such a footer _after_ the body you submitted, so read the body
  ...
  made while the gate is still running leaves `ci-ok` red on the _unfinished
  gate_ branch and costs a second edit. This is a workaround for tooling this
...
- **`gh pr merge` does not work on a stacked PR.** Both the GraphQL path and
  `PUT /repos/{owner}/{repo}/pulls/{n}/merge` refuse with _"This pull request is
  part of a stack and must be merged using the asynchronous merge REST API."_
...
- **Do not retarget a stacked child before merging its parent.** GitHub refuses
  with _"Cannot change the base branch because the pull request is part of a
