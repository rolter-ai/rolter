...
| Added a row to a dashboard sheet form | compose it from the shared form primitives in `ui/src/components/ui/` — `FieldLabel`, `FieldError` + `describedBy`, `FormSection`, `Segmented`, `LockButton`, `ChipGroup`, `SwitchRow` — never a second copy inside the sheet's own file; a screen row is still `Field`; `bun run check:primitives` fails on a component re-declared under a name `ui/` already exports, unless the file imports the shared one and composes it; see `docs/dev-docs/development/form-primitives.md` |
