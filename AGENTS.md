...
| Added a story that asserts a gated control or a sheet's rows | never assert it once: a gated control renders **enabled** until `/api/v1/rbac/effective` answers, so use `expectRefused` rather than a bare `toBeDisabled()`, and a sheet fires its own query as it opens, so its rows want `await findByRole(…)` and not a `getByRole` on the line after the dialog appeared (#1689). `bun run check:waits` fails on both; a control disabled by its own prop from the first paint carries `// story-wait-allow: <reason>` above the line. See `docs/dev-docs/development/testing.md` |
